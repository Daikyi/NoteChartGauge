...
public class GraphUtility{
	...
	public static Scene getNPSGraph(SMDiffRater smdr){

		return chartNPS(smdr.getChartNPS());
	}
	...
	private static Scene chartNPS(ArrayList<Difficulty> nps) {
		
		double max = 0;
		for(int i = 0; i < nps.size(); i++)
			if(nps.get(i).getDifficulty() > max)
				max = nps.get(i).getDifficulty();
        ...
        for(int i = 0; i < nps.size(); i++)
        	seriesNPS.getData().add(new Data<Number, Number>(nps.get(i).getTimeStamp(), nps.get(i).getDifficulty()));
