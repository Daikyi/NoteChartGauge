...
public class BunBotImpl2 extends ListenerAdapter
{
	...
	private static final String SPAM = "289929579169185793"; 
	//private static final String BOMB_CHAN = "153956447649136641";
	private static final String BOMB_CHAN= "153956447649136641";
	public static final String BOT_USERNAME = "BombBot";
    ...
    {
        jda = new JDABuilder(AccountType.BOT).setToken("NDEzNzM2Njc2MzAxNzMzODg4.DWdKCg.qXSdbH55merNK0Lv12e_T6v8kGU").buildBlocking();
        users = Util.loadUserXML();
        jda.addEventListener(new BunBotImpl2());
    }
    ...
    public void onMessageReceived(MessageReceivedEvent mre){
        ...
        if(message.isFromType(ChannelType.PRIVATE)){
        	if(!message.getChannel().equals(SPAM) && !message.getChannel().getId().equals(BOMB_CHAN)){
        		return;
        	}
        }
        if(mre.getAuthor().getName().equals(BOT_USERNAME)
        ...
        if(splitContent[0].equals("roll")){
        	...
        	sendTo.sendMessage(builder.toString()).queue();
        }//end roll
         ...
         */
        ...
        else if((message.getChannel().getId().equals(BOMB_CHAN) || message.getChannel().getId().equals(SPAM)) && !message.isFromType(ChannelType.PRIVATE)){

        	ExplosiveStatus explStatus = Explosives.getStatus();
        	
        	//bombs a user.
	        if(splitContent[0].equals("bomb") && explStatus==ExplosiveStatus.None && !message.isFromType(ChannelType.PRIVATE))
	        ...
	        else if(splitContent[0].equals("missile") && explStatus==ExplosiveStatus.None && !message.isFromType(ChannelType.PRIVATE))
	        ...
	        else if(splitContent[0].equals("tnt") && explStatus==ExplosiveStatus.None && !message.isFromType(ChannelType.PRIVATE))
	        	bombed = Explosives.tnt(message, sendTo);
	        
	        //nukes another user....?
	        else if(splitContent[0].equals("nuke") && !message.isFromType(ChannelType.PRIVATE))
	        	sendTo.sendMessage("shhhh....").queue();
	        
	        //sets up a landmine....?
	        else if(splitContent[0].equals("landmine") && !message.isFromType(ChannelType.PRIVATE))
	        	sendTo.sendMessage("?").queue();
	        ...
	        else if(!message.isFromType(ChannelType.PRIVATE) && Explosives.validChoice(splitContent[0])){
    ...
    private void helpSend(String arg, User u){
    	...
    	RequestFuture<PrivateChannel> localPriv = u.openPrivateChannel().submit();
    	try {
			localPriv.get().sendMessage(sb.toString()).queue();
    ...
    private boolean downloadSM(File file, Attachment a){
        ...
        try{
            ...
            URLConnection con;
            if (((Object) jda).getGlobalProxy() == null)
    ...
    public static UserStat getUser(User u){
    	...
    	if(users.containsKey(u.getId())){
    		...
    		if (!us.getUsername().equals(u.getName()))
    			us.setNewUserName(u.getName());		
    		return users.get(u.getId());
    	}//end if
    	
    	//user does not exist, so create a new entry for them
    	UserStat newUser = new UserStat(u.getName(), u.getId());
