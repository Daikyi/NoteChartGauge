...
public class Explosives {
    ...
    public static UserStat cutWire(Message m, MessageChannel mc){
    	...
    	String wire = m.getContentRaw().substring(1).split(" ")[0].toLowerCase();
    ...
    public static UserStat dodgeMissile(Message m, MessageChannel mc){
    	...
    	String dodge = m.getContentRaw().substring(1).split(" ")[0].toLowerCase();
    	StringBuilder sb = new StringBuilder();
    ...
    public static void bombStats(Message m, User u, HashMap<String, UserStat> users){
    	...
    	try {
			localPriv.get().sendMessage(builder.toString()).queue();
    ...
    public static UserStat missile(Message m, MessageChannel mc){
    	...
    	if(mentions.size() == 0){
    	...
    	else if(mentions.get(0).getUser().getName().equals(BunBotImpl2.BOT_USERNAME)){
    		...
    		target = BunBotImpl2.getUser(mentions.get(0).getUser());
    	else{
    ...
    public static UserStat plunge(Message m, MessageChannel mc){
    	...
    	String plunger = m.getContentRaw().substring(1).split(" ")[0].toLowerCase();
    	StringBuilder sb = new StringBuilder();
    	...
    	if(!plunger.equals(correctChoice)){
    		...
    		explosiveActive = ExplosiveStatus.None;
    		boomSilence(1);
    		mc.sendMessage("THREE").queue();
    		boomSilence(1);
    		mc.sendMessage("TWO").queue();
    		boomSilence(1);
    		mc.sendMessage("ONE").queue();
    		boomSilence(1);
        	//sb.append(Util.getBombSay("",bombed.getLastUsername(),BombAction.Explode)+"\nThe correct wire was " + correctWire);
